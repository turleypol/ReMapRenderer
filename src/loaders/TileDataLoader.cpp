...
cTileDataLoader::cTileDataLoader( std::string filename )
{
  ...
  if ( !tiledatafile )
  {
    pDebug.Log( "Out of Memory in ArtLoader::ArtLoader", __FILE__, __LINE__, LEVEL_ERROR );
    return;
  }
  ...
  tiledataMaxID = TILEDATA_MAX_ID_LAND +
                  ( highseas ? TILEDATA_MAX_ID_STATIC_HS : TILEDATA_MAX_ID_STATIC_PREHS );
}
...
bool cTileDataLoader::LoadEntry( unsigned int index, struct TileDataStaticEntry* entry )
{
  ...
  static const unsigned terrain_sectionsize =
      ( ( TILEDATA_MAX_ID_LAND / entries_per_group ) * terrain_groupsize ) +
      ( ( TILEDATA_MAX_ID_LAND % entries_per_group ) * terrain_entrysize );

  static const unsigned static_entrysize =
      highseas ? sizeof( TileDataStaticEntry ) : sizeof( TileDataStaticEntry_PreHS );  // 41 : 37
  ...
  tiledatafile->seekg( terrain_sectionsize + ( static_groupsize * group_idx ) + 4 +
                           ( idx_in_group * static_entrysize ),
                       ios::beg );
