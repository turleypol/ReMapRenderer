...
cAnimLoader::cAnimLoader( std::string filename, std::string indexname )
{
  ...
  if ( !animfile )
  {
    pDebug.Log( "Out of Memory in AnimLoader::AnimLoader", __FILE__, __LINE__, LEVEL_ERROR );
    return;
  }
...
cAnimation* cAnimLoader::LoadAnimation( int id )
{
  ...
  Uint32 framecount = *( (Uint32*)filep );
  filep += 4;
  Uint32* lookuptable = (Uint32*)filep;
  filep += framecount * 4;
  ...
  for ( int index = 0; index < framecount; index++ )
  {
    ...
    frame->centerx = *( (Sint16*)filep );
    filep += 2;
    frame->centery = *( (Sint16*)filep );
    filep += 2;
    Uint16 width = *( (Uint16*)filep );
    filep += 2;
    Uint16 height = *( (Uint16*)filep );
    filep += 2;
