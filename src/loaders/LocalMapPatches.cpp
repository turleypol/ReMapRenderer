...
void LocalMapPatches::AddPatch( struct StaticPatch patch )
{
  ...
  if ( !entry )
  {
    pDebug.Log( "Out Of Memory in AddPatch::Add(struct StaticPatch)", __FILE__, __LINE__,
                LEVEL_CRITICAL );
    return;
  }
...
void LocalMapPatches::DelPatch( unsigned int blockx, unsigned int blocky, unsigned int obj_id )
{
  ...
  while ( entry )
  {
    if ( ( entry->patch.blockx == blockx ) && ( entry->patch.blocky == blocky ) &&
         ( entry->patch.obj_id == obj_id ) )
      break;
    last = entry;
    entry = entry->next;
  }
...
int LocalMapPatches::GetNewStaticSize( int orig_count, unsigned int blockx, unsigned int blocky,
                                       bool& apply_needed )
{
...
StaticPatch* LocalMapPatches::GetPatch( unsigned int blockx, unsigned int blocky,
                                        unsigned int objid )
{
  struct PatchEntry* entry = root;
  while ( entry )
...
int LocalMapPatches::Load( char* filename )
{
  ...
  if ( !file.is_open() )
  {
    ...
    pDebug.Log( (char*)s.c_str(), __FILE__, __LINE__, LEVEL_WARNING );
    return ( false );
  }
  ...
  {
    file.close();
    pDebug.Log( "Unknown Header ID in LocalMapPatches::Load(char *)", __FILE__, __LINE__,
                LEVEL_WARNING );
    return ( false );
  }
  ...
  struct StaticPatch* patches =
      (struct StaticPatch*)malloc( header.StaticPatchCount * sizeof( struct StaticPatch ) );
...
void LocalMapPatches::ApplyPatch( unsigned int blockx, unsigned int blocky, unsigned int oldcount,
                                  unsigned int newcount, struct staticinfo* src,
                                  struct staticinfo* dst )
{
