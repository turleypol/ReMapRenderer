...
const char* firstNameSet = "_:ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
const char* nameSet = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789\xb7.-_:";

static inline bool isInSet( const char* set, char c )
{
  int i = 0;
  while ( set[i] )
    if ( set[i++] == c )
...
char Parser::parseReference()
{
  ...
  {
    result =
        static_cast<char>( strtol( reference.substr( 2, reference.length() - 2 ).c_str(), 0, 16 ) );
  }
  else if ( reference[0] == '#' )
  {
    result =
        static_cast<char>( strtol( reference.substr( 1, reference.length() - 1 ).c_str(), 0, 10 ) );
  }
