...
void Game::LoadMuls( std::string mulpath )
{
  ...
  pMapLoader = new UOMapLoader( mul_map.c_str(), mul_statics.c_str(), mul_staidx.c_str(),
                                nConfig::widthblocks, nConfig::heightblocks );
...
int Game::Init( void )
{
  ...
  if ( !renderer )
  {
    pDebug.Log( "Renderer Creation failed in Game::Init(void)", __FILE__, __LINE__,
                LEVEL_CRITICAL );
    return ( false );
  }
