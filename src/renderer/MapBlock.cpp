...
static SDL_Surface* GetStretchedSurface( int y2, int y3, int y4, int* alphas, SDL_Surface* texture,
                                         int h, int min_y )
{
  if ( !texture )
    return NULL;
...
bool cMapblock::Generate( int blockx, int blocky )
{
  ...
  for ( xc = 0; xc < 9; ++xc )
    for ( yc = 0; yc < 9; ++yc )
    {
      ...
      if ( alpha_values[xc][yc] > 10.0f )
        alpha_values[xc][yc] = 10.0f;
      if ( alpha_values[xc][yc] < -50.0f )
        alpha_values[xc][yc] = -50.0f;
    }
...
void cMapblock::RenderType( int type, int x, int y, SDL_Surface* target, SDL_Rect* cliprect )
{
  if ( !pTextureBuffer || !target )
    return;
  if ( ( ( outbox.x + outbox.w + x ) < 0 ) || ( ( outbox.y + outbox.h + y ) < 0 ) ||
       ( outbox.x + x >= RENDER_CACHE_WIDTH ) || ( outbox.y + y >= RENDER_CACHE_HEIGHT ) )
    return;
...
void cMapblock::RenderObjectGround( cEntity* object, int x, int y, SDL_Surface* target,
                                    SDL_Rect* cliprect )
{
  ...
  if ( ground->stretch )
  {
    ...
    if ( texture && texture->GetSurface() )
    {
      ...
      bool isOK = true;
      if ( cliprect )
        isOK &= !( ( rect.x > ( cliprect->x + cliprect->w ) ) ||
                   ( rect.y > ( cliprect->y + cliprect->h ) ) ||
                   ( cliprect->x > ( rect.x + rect.w ) ) || ( cliprect->y > ( rect.y + rect.h ) ) );
      if ( isOK )
      {
        SDL_Surface* surface =
            GetStretchedSurface( y2, y3, y4, alphas, texture->GetSurface(), rect.h, min_y );
        if ( !surface )
        {
...
void cMapblock::RenderObjectItem( cEntity* object, int x, int y, SDL_Surface* target,
                                  SDL_Rect* cliprect )
{
  ...
  bool clip = false;
  if ( cliprect )
    clip = ( rect.x > ( cliprect->x + cliprect->w ) ) ||
           ( rect.y > ( cliprect->y + cliprect->h ) ) || ( cliprect->x > ( rect.x + rect.w ) ) ||
           ( cliprect->y > ( rect.y + rect.h ) );
  if ( !clip )
  {
