    ...
    main( int argc, char* argv[] )
{
  ...
  const int t_width =
      ( (int)( ( renderwidth + divisor * 4 - 1 ) / ( divisor * 4 ) ) ) * divisor * 4;
  const int t_height =
      ( (int)( ( renderheight + divisor * 4 - 1 ) / ( divisor * 4 ) ) ) * divisor * 4;
  ...
  Uint8 bmpheader[54] = { 0x42, 0x4D, 0xCE, 0x3A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x36,
                          0x00, 0x00, 0x00, 0x28, 0x00, 0x00, 0x00, 0x64, 0x00, 0x00, 0x00,
                          0x32, 0x00, 0x00, 0x00, 0x01, 0x00, 0x18, 0x00, 0x00, 0x00, 0x00,
                          0x00, 0x98, 0x3A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  ...
  *bmpheight = t_height / divisor;
  ...
  const size_t realmem =
      sizeof( bmpheader ) + ( t_pixels * 3 );  // * 3 -> a pixel has a byte for R, for G, for B
  printf( "Creating output Bitmap file (%u x %u): %zu bytes (Zoom 1/%i)...", ( *bmpwidth ),
          ( *bmpheight ), realmem, divisor );
  ...
  printf( "Area to render (X/Y): (%i/%i) to (%i/%i).\n", nConfig::minx, nConfig::miny,
          nConfig::maxx, nConfig::maxy );
  ...
  for ( int blocky = 0; blocky <= bycount; ++blocky )
    for ( int blockx = 0; blockx <= bxcount; ++blockx )
    {
      int xcount = xsize;
      if ( blockx == bxcount )
        xcount = ( t_width % rc_width ) / divisor;
      int ycount = ysize;
      if ( blocky == bycount )
        ycount = ( t_height % rc_height ) / divisor;
        ...
        for ( int i = 0; i < detail; ++i )
        {
          ...
          for ( int y = 0; y < h; ++y )
          {
            ...
            for ( int x = 0; x < w; ++x )
            {
              ...
              *( dst + 1 ) =
                  ( *( src + 1 ) + *( src + 4 ) + *( src + w * 6 + 1 ) + *( src + w * 6 + 4 ) ) / 4;
              *( dst + 2 ) =
                  ( *( src + 2 ) + *( src + 5 ) + *( src + w * 6 + 2 ) + *( src + w * 6 + 5 ) ) / 4;
              src += 6;
              dst += 3;
            }
          }
      ...
      for ( int y = 0; y < ycount; ++y )
      {
        ...
        file.seekp( sizeof( bmpheader ) +
                        ( t_width / divisor * ( t_height / divisor - 1 - blocky * ysize - y ) +
                          blockx * xsize ) *
                            3,
                    ios::beg );
